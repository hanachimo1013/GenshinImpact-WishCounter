<template>
  <div class="min-h-screen gradient-pink">
    <loading-spinner v-if="isLoadingStats" />
    <!-- Header -->
    <div class="bg-black/40 backdrop-blur-md border-b border-pink-400/20 sticky top-0 z-10">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 flex items-center justify-between">
        <router-link to="/" class="text-2xl sm:text-3xl hover:text-pink-200 transition text-white">← Back</router-link>
        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white hidden sm:block">👤 Character Event Wishes</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12 lg:py-16">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8 mb-6 sm:mb-12 lg:mb-16">
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-4 sm:p-6 lg:p-8 text-center rounded-xl">
          <p class="text-pink-200 text-xs sm:text-sm lg:text-lg mb-1 sm:mb-2">Total Wishes</p>
          <p class="text-3xl sm:text-4xl lg:text-5xl font-bold text-white">{{ totalWishes }}</p>
        </div>
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-4 sm:p-6 lg:p-8 text-center rounded-xl">
          <p class="text-pink-200 text-xs sm:text-sm lg:text-lg mb-1 sm:mb-2">5-Star Pulls</p>
          <p class="text-3xl sm:text-4xl lg:text-5xl font-bold text-yellow-300">{{ fiveStarPulls }}</p>
        </div>
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-4 sm:p-6 lg:p-8 text-center rounded-xl">
          <p class="text-pink-200 text-xs sm:text-sm lg:text-lg mb-1 sm:mb-2">Pity Counter</p>
          <p class="text-3xl sm:text-4xl lg:text-5xl font-bold text-pink-300">{{ pityCounter }}</p>
        </div>
      </div>
      ...
      <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-4 sm:p-6 lg:p-8 rounded-xl mb-6 sm:mb-12 lg:mb-16">
        ...
        <div class="flex flex-col sm:flex-row gap-2 sm:gap-4">
          <input 
            ...
            max="5"
            @keyup.enter="addWish"
            class="flex-1 px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base bg-pink-950/50 border border-pink-400/30 rounded-lg text-white placeholder-pink-300 focus:outline-none focus:border-pink-300 transition"
          />
          <button 
            @click="addWish"
            class="btn-primary px-4 sm:px-8 py-2 sm:py-3 text-sm sm:text-base bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 whitespace-nowrap"
            :disabled="isLoading"
          >
      ...
      <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-4 sm:p-6 lg:p-8 rounded-xl">
        ...
        <div v-if="wishes.length === 0" class="text-center py-8 sm:py-12">
          <p class="text-pink-300 text-sm sm:text-base lg:text-lg">No wishes yet. Start tracking! ✨</p>
        </div>
        <div v-else class="space-y-2 sm:space-y-3">
          ...
          >
            <div class="flex justify-between items-center flex-col sm:flex-row gap-2 sm:gap-0">
              ...
              <span class="text-pink-300 text-xs sm:text-sm">{{ formatDate(wish.timestamp) }}</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async addWish() {
      ...
      const { data, error } = await wishService.addWish(2, this.rarity)
      if (error) {
