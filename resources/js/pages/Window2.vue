  ...
  <div class="min-h-screen gradient-pink">
    ...
    <div class="bg-black/40 backdrop-blur-md border-b border-pink-400/20 sticky top-0 z-10">
      <div class="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
        <router-link to="/" class="text-3xl hover:text-pink-200 transition text-white">← Back</router-link>
        <h1 class="text-3xl font-bold text-white">👤 Character Event Wishes</h1>
        <div class="w-12"></div>
      </div>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-6 py-12">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-8 text-center rounded-xl">
          <p class="text-pink-200 text-lg mb-2">Total Wishes</p>
          <p class="text-5xl font-bold text-white">{{ totalWishes }}</p>
        </div>
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-8 text-center rounded-xl">
          <p class="text-pink-200 text-lg mb-2">5-Star Pulls</p>
          <p class="text-5xl font-bold text-yellow-300">{{ fiveStarPulls }}</p>
        </div>
        <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-8 text-center rounded-xl">
          <p class="text-pink-200 text-lg mb-2">Pity Counter</p>
          <p class="text-5xl font-bold text-pink-300">{{ pityCounter }}</p>
        </div>
      </div>
      ...
      <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-xl mb-12">
        ...
        <div class="flex gap-4">
          <input 
            ...
            min="3" 
            max="5"
            class="flex-1 px-4 py-3 bg-pink-950/50 border border-pink-400/30 rounded-lg text-white placeholder-pink-300 focus:outline-none focus:border-pink-300 transition"
          />
          <button 
            @click="addWish"
            class="btn-primary px-8 py-3 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700"
          >
            Add
          </button>
        </div>
      ...
      <div class="card bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-xl">
        ...
        <div v-if="wishes.length === 0" class="text-center py-12">
          <p class="text-pink-300 text-lg">No wishes yet. Start tracking! ✨</p>
        </div>
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex justify-between items-center">
              ...
              <span class="text-pink-300 text-sm">{{ wish.timestamp }}</span>
            </div>
          </div>
        </div>
      </div>
